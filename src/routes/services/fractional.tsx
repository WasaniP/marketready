...
function LaunchTrajectory() {
 ...
 return (
  <div
   ref={ref}
   className="relative w-full max-w-4xl overflow-hidden rounded-2xl border border-hairline bg-[#1F1A16]/60 px-4 pb-5 pt-6 sm:px-8"
  >
   <div
    aria-hidden="true"
    className="pointer-events-none absolute -top-24 left-1/2 h-40 w-[36rem] -translate-x-1/2 rounded-full bg-ember/[0.06] blur-3xl"
   />
   <svg viewBox="0 0 920 220" className="relative h-auto w-full" role="img" aria-label="Launch trajectory: from today's low base through Pre-launch, Launch, and Beyond">
    ...
    <g className={reveal(0)} style={{ animationDelay: "0.5s" }}>
     ...
     <text x="196" y="126" fontSize="13.5" fontWeight={700} fill="#B8AEA3" textAnchor="middle">
      Pre-launch
     </text>
    ...
    <g className={reveal(1)} style={{ animationDelay: "0.72s" }}>
     ...
     <circle cx="466" cy="96" r="6" fill={TEAL} style={{ }} />
    ...
    <g className={reveal(2)} style={{ animationDelay: "0.94s" }}>
     ...
     <circle cx="764" cy="44" r="6.5" fill={TEAL} style={{ }} />
   ...
   <p className="relative mt-1 text-center text-[11px] uppercase tracking-widest text-fog">
...
const DIAL_TICKS = [
 ...
 { f: 0.5, name: "Co-Pilot", color: INDIGO, glow: "rgba(232,201,160,0.25)", font: "#B8AEA3", bold: 700 },
 { f: 0.9, name: "Full Velocity", color: TEAL, glow: "rgba(201,106,66,0.95)", font: INK, bold: 700 },
] as const;
...
function EngagementEscalator() {
 ...
 return (
  <div ref={ref} className="relative mx-auto mt-10 max-w-3xl">
   ...
   <div
    aria-hidden="true"
    className="mr-glow-pulse absolute left-1/2 top-[26%] h-56 w-[26rem] -translate-x-1/2 rounded-full bg-ember/20 blur-3xl"
   />
   <svg
    viewBox="0 0 360 200"
   ...
   >
    ...
    <path
     ...
     style={{ transitionDelay: visible ? "0.2s" : "0ms" }}
    />
    {/* Tier ticks */}
    {DIAL_TICKS.map((t) => {
...
function TimeToValuePaths() {
 ...
 return (
  <div ref={ref} className="relative mx-auto mt-10 max-w-4xl">
   <div
    aria-hidden="true"
    className="pointer-events-none absolute inset-x-0 top-[30%] h-24 rounded-full bg-ember/[0.05] blur-2xl"
   />
   <svg viewBox="0 0 920 330" className="relative h-auto w-full" role="img" aria-label="Three time to value paths: Full-Time Hire takes months on a winding road, Traditional Agency takes weeks with an account manager detour, MarketReady takes days on a direct road">
    ...
    <circle cx="236" cy="268" r="10" fill={SCOREWORK} opacity="0.14" />
    <circle cx="236" cy="268" r="5" fill={SCOREWORK} style={{ }} />
    <circle cx="243" cy="262" r="3.4" fill="none" stroke={SCOREWORK} strokeWidth="1.1" />
    <path d="M 237 266 a 4.4 3.2 0 0 1 6.4 0" fill="none" stroke={SCOREWORK} strokeWidth="1.1" />
    <text x="258" y="272" fontSize="11" fill={SCOREWORK} className={step(2)} style={{ animationDelay: "0.5s" }}>
     account manager
    </text>
    ...
    <path
     ...
     strokeLinecap="round"
     className="mr-gauge-sweep"
     style={{ transition: "stroke-width 0.6s ease, opacity 0.6s ease", opacity: visible ? 1 : 0.35 }}
    />
...
function FitFork() {
 ...
 return (
  <div ref={ref} className="relative mx-auto mt-12 max-w-4xl">
   <div
    aria-hidden="true"
    className="pointer-events-none absolute left-[10%] top-4 h-40 w-[70%] rounded-full bg-ember/[0.04] blur-3xl"
   />
   <svg viewBox="0 0 960 560" className="relative h-auto w-full" role="img" aria-label="Fork in the road: qualified visitors take the teal path to Full Velocity, others take the coral path to try the Sprint or Audit first">
    ...
    <path
     ...
     style={{ strokeDasharray: visible ? undefined : "0 400", transition: "opacity 0.9s ease" }}
    />
    {/* Full Velocity destination badge */}
    <g className={step(2)}>
     <rect x="776" y="38" width="172" height="46" rx="23" fill="#16120F" stroke={TEAL} strokeOpacity="0.55" style={{ }} />
    ...
    <g className={step(3)}>
     <rect x="700" y="500" width="248" height="48" rx="24" fill="#16120F" stroke={SCOREWORK} strokeOpacity="0.45" style={{ }} />
     <path d="M 756 508 C 774 512 786 516 792 520" fill="none" stroke={SCOREWORK} strokeOpacity="0.5" strokeWidth="1.5" strokeDasharray="3 4" />
    ...
    {NOT_FIT_CAPTIONS.map((c, i) => (
     <g key={c} className={step(1)}>
      <circle cx="388" cy={418 + i * 26} r="3" fill={SCOREWORK} />
...
function CompareLabel({ children }: { children: string }) {
 return (
  <div className="flex items-center px-4 py-4 text-left text-[11px] font-semibold uppercase tracking-widest text-fog">
   {children}
  </div>
 );
}
...
function CompareCell({ tone, text }: { tone: "strong" | "weak" | "ok"; text: string }) {
 ...
 const mark = hero ? (
  ...
  <CheckIcon className="h-3.5 w-3.5 shrink-0 text-fog/70" />
 );
 return (
  <div
   className={`flex items-center justify-center gap-2 px-3 py-4 text-center text-sm leading-snug ${
    hero ? "border-x border-ember/50 bg-ember/[0.06]" : ""
   }`}
  >
   {mark}
   <span className={hero ? "font-semibold text-pinetint" : weak ? "text-mist/70" : "text-fog"}>
    {text}
   </span>
  </div>
 );
   ...
   <main>
    ...
    <section className="border-b border-hairline bg-[#16120F] px-5 py-10 sm:px-8 sm:py-14">
     <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-12 overflow-x-auto">
       <div className="mx-auto min-w-[44rem] max-w-5xl overflow-hidden rounded-2xl border border-hairline bg-[#1F1A16]/60 ">
        {/* Header */}
        <div className="grid grid-cols-[9rem_1fr_1.15fr_1fr] items-stretch border-b border-hairline">
         <div className="px-4 py-5" />
         <div className="flex items-center justify-center px-3 py-5 text-center text-sm font-semibold uppercase tracking-widest text-fog">
          Full-Time Hire
         </div>
         <div className="relative flex items-center justify-center border-x border-ember/50 bg-ember/[0.06] px-3 py-5 text-center text-sm font-bold uppercase tracking-widest text-ember ">
          MarketReady
         </div>
         <div className="flex items-center justify-center px-3 py-5 text-center text-sm font-semibold uppercase tracking-widest text-fog">
          Traditional Agency
       ...
       {TIERS.map((tier, i) => (
        <div
         ...
         className={`relative flex flex-col rounded-xl border p-7 transition-all duration-300 hover:-translate-y-0.5 ${
          tier.featured
           ? "border-ember/50 bg-gradient-to-b from-linen/70 to-linen/40 "
           : "border-hairline bg-linen/60 hover:border-hairline"
         }`}
        >
         {tier.featured && (
          <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full border border-ember/50 bg-[#1F1A16] px-3 py-0.5 text-[11px] font-bold uppercase tracking-wider text-ember">
           {tier.label}
          </span>
         )}
         <span
          className={`flex h-10 w-10 items-center justify-center rounded-lg ${
           tier.featured ? "bg-ember/15 text-ember" : i === 1 ? "bg-indigo/15 text-indigo" : "bg-ember/5 text-ember"
          }`}
         >
          <CardIcon name={i === 0 ? "message" : i === 1 ? "rocket" : "spark"} />
         </span>
         <h3 className="mt-4 text-base font-semibold leading-tight text-ink">{tier.title}</h3>
         <p className="mt-2 text-sm font-bold text-ember">{tier.price}</p>
         ...
         <div className="mt-6 border-t border-hairline pt-4">
          <span className="inline-block rounded-full border border-ember/40 px-3 py-1 text-xs font-medium text-ember">
           {tier.tag}
          </span>
         </div>
        </div>
    ...
    <section className="border-b border-hairline bg-[#1F1A16] px-5 py-10 sm:px-8 sm:py-14">
     <div className="mx-auto max-w-6xl">
      ...
      />
      <div className="mt-12 overflow-hidden rounded-xl border border-hairline bg-linen/60 backdrop-blur-md">
        ...
        {DELIVERABLES.map((card, i) => {
         ...
         return (
          <div key={card.title} className="flex flex-col p-7">
           <span
            className={`flex h-10 w-10 items-center justify-center rounded-lg ${
             isTeal ? "bg-ember/15 text-ember" : "bg-indigo/15 text-indigo"
            }`}
           >
           ...
           <ul className="mt-4 flex flex-col gap-2.5">
            {card.items.map((item) => (
             <li key={item} className="flex items-start gap-2.5 text-sm leading-relaxed text-mist">
              <CheckIcon
               className={`mt-0.5 h-4 w-4 shrink-0 ${isTeal ? "text-ember" : "text-indigo"}`}
              />
              <span>{item}</span>
             </li>
            ))}
           ...
           <div className="mt-6 border-t border-hairline pt-4">
            <span
             className={`inline-block rounded-full border px-3 py-1 text-xs font-medium ${
              isTeal ? "border-ember/40 text-ember" : "border-indigo/40 text-indigo"
             }`}
            >
             {card.tag}
            </span>
    ...
    <section className="bg-[#16120F] px-5 py-12 sm:px-8 sm:py-14">
     <div className="mx-auto max-w-2xl rounded-xl border border-indigo/30 bg-linen/60 p-8 text-center sm:p-10">
