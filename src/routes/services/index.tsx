...
const CLUSTER_ACCENTS = [
 {
  hex: "#C96A42",
  soft: "rgba(201,106,66,0.18)",
  text: "#E8C9A0",
  fill: "rgba(201,106,66,0.10)",
  glow: "rgba(201,106,66,0.12)",
 },
 {
  hex: "#E8C9A0",
  soft: "rgba(232,201,160,0.20)",
  text: "#F5F0E8",
  fill: "rgba(232,201,160,0.10)",
  glow: "rgba(232,201,160,0.12)",
 },
 {
  hex: "#6E9464",
  soft: "rgba(110,148,100,0.22)",
  text: "#B8AEA3",
  fill: "rgba(110,148,100,0.12)",
  glow: "rgba(110,148,100,0.14)",
 },
];
  ...
  >
   <defs>
    <linearGradient id="funnelFade" x1="0" y1="0" x2="0" y2="1">
     <stop offset="0%" stopColor="#C96A42" stopOpacity="0.12" />
     <stop offset="100%" stopColor="#C96A42" stopOpacity="0.7" />
    </linearGradient>
   </defs>
   {[120, 360, 600].map((x) => (
    <g key={x}>
...
function ScoreDial({ score = 68 }: { score?: number }) {
 ...
 return (
  <div className="relative flex h-44 w-44 items-center justify-center sm:h-48 sm:w-48">
   <div aria-hidden="true" className="mr-glow-pulse absolute -inset-3 rounded-full bg-ember/25 blur-2xl" />
   <svg
    viewBox="0 0 160 160"
   ...
   <div className="absolute inset-0 flex flex-col items-center justify-center">
    <span className="text-[2.5rem] font-extrabold leading-none tabular-nums text-pinetint drop-">
     {score}
    </span>
   ...
   <main>
    ...
    <section className="relative overflow-hidden border-b border-slate-800 bg-slate-950 px-5 pb-14 pt-32 sm:px-8 sm:pb-16 sm:pt-40">
     <div
      ...
      style={{
       backgroundImage:
        "radial-gradient(ellipse 40rem 22rem at 20% -10%, rgba(201,106,66,0.25), transparent 60%), radial-gradient(ellipse 36rem 20rem at 85% 5%, rgba(201,106,66,0.25), transparent 60%), radial-gradient(ellipse 30rem 18rem at 50% 120%, rgba(201,106,66,0.25), transparent 60%)",
      }}
     />
     <div
      aria-hidden="true"
     ...
     <div className="relative mx-auto max-w-4xl text-center">
      <span className="inline-block rounded-full border border-teal-500/40 bg-teal-500/10 px-3 py-1 text-xs font-medium text-teal-300 ">
       [ SERVICES OVERVIEW ]
      </span>
      ...
      <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
       ...
       <a
        href="/services/audit"
        className="inline-flex items-center justify-center rounded-lg border border-hairline px-7 py-3.5 text-base font-medium text-mist transition-colors hover:bg-linen/40"
       >
        View Audit Details
       </a>
      </div>
    ...
    <section className="relative overflow-hidden border-b border-slate-800 bg-slate-950 px-5 py-10 sm:px-8 sm:py-14">
     ...
     <div className="relative mx-auto max-w-6xl">
      ...
      <div className="mt-12 hidden md:block overflow-x-auto">
       <table className="w-full text-left border border-teal-500/10 bg-[#2A2320]/40 backdrop-blur-md">
        <thead>
         <tr className="border-b border-slate-800/80">
          {["Service Tier", "Deliverables & Scope", "Speed", "Best For", "Action"].map(
           (h) => (
            <th
             key={h}
             className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-fog"
            >
             {h}
            </th>
           ),
         ...
         {TIERS.map((tier) => (
          <tr
           ...
           className={`border-b border-slate-800/50 align-top transition-colors hover:bg-teal-500/[0.03] ${
            tier.recommended
             ? "border-t border-t-teal-400/50 border-b-teal-400/30 bg-teal-500/[0.06] "
             : "border-l-4 border-l-transparent"
           }`}
          >
           <td className="px-4 py-5">
            {tier.recommended && (
             <span className="mb-2 inline-block rounded-full border border-teal-400/70 bg-teal-500/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-pinetint ">
              [ HERO RECOMMENDED ]
             </span>
            )}
            ...
            <div
             className={`mt-0.5 text-sm ${
              tier.recommended ? "font-bold text-teal-400" : "text-fog"
             }`}
            >
             {tier.price}
            </div>
           ...
           <td className="px-4 py-5">
            {tier.ctaStyle === "solid" ? (
             ...
             <a
              href={tier.href}
              className="inline-block rounded-lg border border-hairline px-4 py-2 text-sm font-medium text-mist transition-colors hover:bg-linen/40"
             >
              {tier.cta}
             </a>
            )}
       ...
       {TIERS.map((tier) => (
        <div
         ...
         className={`rounded-2xl border p-5 backdrop-blur-md transition-colors ${
          tier.recommended
           ? "border-teal-400/70 bg-gradient-to-b from-teal-500/10 to-linen/40 "
           : "border-teal-500/15 bg-[#2A2320]/30 "
         }`}
        >
         {tier.recommended && (
          <span className="mb-2 inline-block rounded-full border border-teal-400/70 bg-teal-500/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-pinetint ">
         ...
         <div className="flex items-baseline justify-between gap-3">
          ...
          <span
           className={`shrink-0 text-sm font-bold ${
            tier.recommended ? "text-teal-400" : "text-fog"
           }`}
          >
           {tier.price}
          </span>
         ...
         <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-fog">
          <span
           className={`inline-block rounded-full border px-2.5 py-1 text-[11px] font-semibold ${tier.pillClass}`}
          >
           {tier.speed}
          ...
          <span>
           <span className="font-medium text-mist">Best for: </span>
           {tier.bestFor}
          </span>
         </div>
         <div className="mt-4">
          {tier.ctaStyle === "solid" ? (
           ...
           <a
            href={tier.href}
            className="inline-block rounded-lg border border-hairline px-5 py-2.5 text-sm font-medium text-mist transition-colors hover:bg-linen/40"
           >
            {tier.cta}
           </a>
          )}
     ...
     <div className="relative mx-auto max-w-6xl">
      ...
      <div className="relative mt-12">
       ...
       <p
        ...
        aria-hidden="true"
       >
        9 Signals <span className="text-ember">→</span> 3 Pillars{" "}
        <span className="text-ember">→</span> 1 Score
       </p>
       ...
       <div className="relative z-10 grid grid-cols-1 gap-5 md:grid-cols-3 md:gap-6">
        {PARAM_NODES.map((node, i) => {
         ...
         return (
          <div
           key={node.num}
           className="relative overflow-hidden rounded-2xl border bg-gradient-to-b from-white/[0.03] to-linen/40 p-5 backdrop-blur-md"
          ...
          >
           ...
           <div className="flex items-center justify-between gap-2">
            ...
            <span className="inline-flex shrink-0 rounded-full border border-hairline bg-white/[0.02] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-fog">
       ...
       <div className="relative z-10 flex flex-col items-center">
        <span className="rounded-full border border-ember/30 bg-ember/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-teal-300">
         1 Composite Score
        </span>
        <div className="mt-4">
         <ScoreDial score={68} />
