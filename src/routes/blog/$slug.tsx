  ...
  switch (block.type) {
        ...
        <ul key={key} className="my-6 flex flex-col gap-2.5">
          {block.items.map((item, i) => (
            <li key={i} className="flex items-start gap-3 text-[17px] leading-7 text-mist">
              <svg
                aria-hidden="true"
                className="mt-2 h-4 w-4 shrink-0 text-electric"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
    ...
    case "quote":
      return (
        <blockquote
          key={key}
          className="my-8 rounded-r-xl border-l-2 border-electric bg-electric/[0.06] px-6 py-5"
        >
          <p className="text-lg italic leading-relaxed text-ink">{block.text}</p>
          {block.cite && (
            <footer className="mt-3 text-sm font-medium not-italic text-zinc-500">
              - {block.cite}
            </footer>
          )}
        </blockquote>
      ...
      <main>
        <article className="mx-auto max-w-[720px] px-5 pb-16 pt-32 sm:px-8 sm:pt-40">
          ...
          <header className="border-b border-hairline pb-8">
            <span className="inline-flex rounded-full border border-electric/40 bg-electric/10 px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-electric">
              {post.tags[0]}
            </span>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-zinc-500">
              ...
              {post.source && post.source.url && (
                <a
                  ...
                  className="text-electric transition-colors hover:text-electric/80"
                >
  ...
  return (
    <div className="min-h-dvh bg-gradient-to-b from-[#0F172A] via-[#111827] to-[#030712]">
      ...
      <main className="mx-auto max-w-2xl px-5 pb-24 pt-40 text-center sm:px-8">
        <span className="chip border-electric/40 text-electric">404</span>
