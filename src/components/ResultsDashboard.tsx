 ...
 return (
  ...
  >
    ...
    <span
     className={`shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${
      card.isAI
       ...
       : "border-hairline bg-white/[0.04] text-fog"
     }`}
    >
   ...
   <div className="flex flex-wrap items-center gap-2">
    ...
    <span className="text-xs font-medium text-fog">/100</span>
...
function PillarBadge({ title, score }: { title: string; score: number | null }) {
 ...
 return (
  <div className="glass-card flex flex-col gap-1.5 px-5 py-4">
   <span className="text-[10px] font-bold uppercase tracking-wider text-fog">{title}</span>
   {score == null ? (
    <span className="text-2xl font-extrabold leading-none text-fog">N / A</span>
   ) : (
...
function PdfCaptureBar({
 ...
 return (
  <div className="mt-10 overflow-hidden rounded-xl border border-ember/40 bg-[#2A2320]/50 backdrop-blur-md">
...
function BriefingOffer({ onBookBriefing }: { onBookBriefing: () => void }) {
 return (
  <div className="mt-6 rounded-xl border border-ember/40 bg-ember/[0.06] p-6 sm:p-8">
   <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
    <div className="max-w-2xl">
     <span className="chip border-ember/40 text-ember">Next step</span>
  ...
  <section id="results" className="scroll-mt-24" aria-live="polite">
   ...
   <div className="flex flex-wrap items-start justify-between gap-4">
    <div>
     <span
      className={`chip ${
       isPreliminary
        ? "border-[#6E9464]/50 bg-[#6E9464]/10 text-[#B8AEA3]"
    ...
    <p className="text-xs text-fog">Scored {formatDate(result.generatedAt)}</p>
   </div>
   ...
   <div className="mt-8">
    <div className="flex items-center gap-3">
     ...
     {aiResult && <span className="chip border-ember/40 text-ember">Final AI Assessment</span>}
    </div>
    <p className="mt-1 max-w-3xl text-sm text-mist">
